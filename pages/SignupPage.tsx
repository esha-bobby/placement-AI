...
import { signup as apiSignup, googleLogin, appleLogin } from '../services/mockApi';
...
const SignupPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const user = await apiSignup(username, email, password, role);
            auth.login(user);
            setLoading(false);
    ...
    const handleSocialSignup = async (provider: 'google' | 'apple') => {
        ...
        try {
            ...
            // In this mock version, we'll assume social signups are always for students.
            auth.login(user);
            setLoading(false);
            navigate('/student/dashboard');
        } catch (err: any) {
            ...
            <div className="w-full max-w-md space-y-8">
                <div className="bg-brand-surface border border-brand-border rounded-2xl p-8 shadow-lg">
                    <div className="text-center">
                        <h2 className="mt-2 text-center text-3xl font-extrabold text-brand-text-primary">
                            Create Account
                        </h2>
                    ...
                    <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
                        ...
                        <div className="flex items-center justify-center space-x-4">
                            ...
                            >
                                <UserIcon className="mr-2" size={16}/> I'm a Student
                            </button>
                            <button
                                type="button"
                            ...
                            >
                                <Briefcase className="mr-2" size={16}/> I'm an Admin
                            </button>
                        </div>
                        
                        <div className="rounded-md shadow-sm -space-y-px">
                            <div className="relative">
                                <input id="username" name="username" type="text" required className="appearance-none rounded-md relative block w-full px-3 py-3 border border-brand-border bg-brand-secondary placeholder-brand-text-secondary text-brand-text-primary focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm pl-10" placeholder="Choose a username" value={username} onChange={(e) => setUsername(e.target.value)} />
                                <UserIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-brand-text-secondary" size={20} />
                            </div>
                            <div className="relative pt-4">
                                <input id="email-address" name="email" type="email" autoComplete="email" required className="appearance-none rounded-md relative block w-full px-3 py-3 border border-brand-border bg-brand-secondary placeholder-brand-text-secondary text-brand-text-primary focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm pl-10" placeholder="Enter your email" value={email} onChange={(e) => setEmail(e.target.value)} />
                                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-brand-text-secondary" size={20} />
                            </div>
                             <div className="relative pt-4">
                                <input id="password" name="password" type={showPassword ? 'text' : 'password'} required className="appearance-none rounded-md relative block w-full px-3 py-3 border border-brand-border bg-brand-secondary placeholder-brand-text-secondary text-brand-text-primary focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm pl-10" placeholder="Create a strong password" value={password} onChange={(e) => setPassword(e.target.value)} />
                             ...
                             <div className="relative pt-4">
                                <input id="confirm-password" name="confirm-password" type={showConfirmPassword ? 'text' : 'password'} required className="appearance-none rounded-md relative block w-full px-3 py-3 border border-brand-border bg-brand-secondary placeholder-brand-text-secondary text-brand-text-primary focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm pl-10" placeholder="Confirm your password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
                                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-brand-text-secondary" size={20} />
                                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-brand-text-secondary hover:text-brand-text-primary">
                    ...
                    <div className="mt-6">
                        ...
                        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <button onClick={() => handleSocialSignup('google')} type="button" aria-label="Sign up with Google" className="w-full flex items-center justify-center py-3 px-4 border border-brand-border rounded-md shadow-sm text-sm font-medium text-brand-text-primary bg-brand-secondary hover:bg-gray-700 transition-colors">
                                <GoogleIcon className="h-5 w-5 mr-2" />
                                <span className="truncate">Google</span>
                            </button>
                            <button onClick={() => handleSocialSignup('apple')} type="button" aria-label="Sign up with Apple" className="w-full flex items-center justify-center py-3 px-4 border border-brand-border rounded-md shadow-sm text-sm font-medium text-brand-text-primary bg-brand-secondary hover:bg-gray-700 transition-colors">
